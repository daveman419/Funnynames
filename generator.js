...
function playSound(name) {
  // Ensure the audio path is correct relative to index.html
  var audio = new Audio("sounds/confetti1.mp3");
  audio.play();
}

$(".btn").click(function(){
    var randomName = generateStupidName();
    const emojis = ["✨","💫","🌈","🤩","🎈","🦄", "🔮", "🧙‍♀️", "🦉", "🪶", "🏰", "👑", "🌿", "🍄", "🕯️", "🎃", "🍎", "🍫", "👻", "📖"];
    const emoji = emojis[Math.floor(Math.random() * emojis.length)];
    $(".output").html(`Your name is : <br>${emoji}<strong>${randomName}</strong>${emoji}`);

    //$(".output").html("Your name is: " + "<br><strong>" + randomName + "</strong>");
    
    playSound();
    
    //confetti!! for simple one use confetti();
